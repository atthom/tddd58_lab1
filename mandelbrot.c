...
struct node_list pop_stack() {
    struct node_list current = node_list;
...
parallel_mandelbrot(struct mandelbrot_thread *args, struct mandelbrot_param *parameters) {
    ...
    struct node_list currentNode = struct node_list; 
    
    while(currentNode !=null) {
        parameters->begin_h = currentNode->x;
        parameters->end_h = currentNode->x + SIZE_ROW;
 
        parameters->begin_w = currentNode->y;
        parameters->end_w =  currentNode->y + SIZE_COL;
